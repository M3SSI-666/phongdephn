...
function QuyHomestayInner({ overrideUserId, overrideRole, isViewAs = false } = {}) {
  ...
  function parseGiaValue(gia) {
    ...
    const k = s.match(/([\d.]+)k/); if (k) return parseFloat(k[1]) / 1000;
  ...
  function buildFilterSummary(f) {
    ...
    else if (f.Gia_Min != null) parts.push(`≥ ${f.Gia_Min}tr`);
    return parts.join(' · ') || 'Tìm kiếm thông minh';
  }
  ...
  function normalizeFilter(f) {
    ...
    if (r.Noi_That != null) {
      const s = r.Noi_That.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
      if (s.includes('full')) r.Noi_That = 'full';
      else if (s.includes('khong') || s.includes('tho')) r.Noi_That = 'không';
      else r.Noi_That = 'cơ bản';
    }
    return r;
  }
  ...
  const filtered = useMemo(() => {
    ...
    if (aiFilter) {
      ...
      if (aiFilter.Thiet_Ke) {
        // Map Thiet_Ke (dạng "2PN") sang Loai_Can ("2 PN")
        list = list.filter(it => (it.Loai_Can || '').toUpperCase().replace(/\s+/g, '').includes(aiFilter.Thiet_Ke.replace(/\s+/g, '')));
      }
      if (aiFilter.Noi_That) list = list.filter(it => (it.Noi_That || '').toLowerCase().includes(aiFilter.Noi_That));
      if (aiFilter.Gia_Max != null) list = list.filter(it => { const g = parseGiaValue(it.Gia_Thang); return g == null || g <= aiFilter.Gia_Max; });
      if (aiFilter.Gia_Min != null) list = list.filter(it => { const g = parseGiaValue(it.Gia_Thang); return g == null || g >= aiFilter.Gia_Min; });
    }
    ...
    <div style={{ fontFamily: F, color: C.text }}>
      ...
      <div style={{ marginBottom: aiFilter ? 8 : 16 }}>
        <div style={{ position: 'relative' }}>
          ...
          <input
            type="text"
            placeholder="VD: S1-08-05A · 2PN dưới 15tr · full nội thất..."
            value={aiQuery}
